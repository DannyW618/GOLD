"""
This file contains the utility functions for the diffusion model, adapted from Neural Graph Generator
https://github.com/iakovosevdaimon/Neural-Graph-Generator/tree/main
"""
...
def p_losses(denoise_model, x_start, t, cond, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, noise=None, loss_type="l1"):
    ...
    return loss, predicted_noise, x_noisy
...
@torch.no_grad()
def draw_samples(initial_samp, model, cond, timesteps, betas, shape, device):
    model.eval()
    b = shape[0]
    samples = []
    samp = initial_samp
    ...
    for i in reversed(range(0, timesteps)):
        samp = p_sample(model, samp, torch.full((b,), i, device=device, dtype=torch.long), cond, i, betas)
