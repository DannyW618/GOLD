...
@torch.no_grad()
def evaluate_classify(model, dataset, eval_func, criterion, args, device, training_data = True, decoder = None, test_only = None):
    model.eval()
    if test_only:
    ...
    if training_data:
        ...
        test_score = eval_func(y[dataset.node_idx], out[dataset.node_idx])

    if test_only:
        return test_score
    
    if (args.method != 'GPN') & training_data:
...
def evaluate_detect(model, dataset_ind, dataset_ood, criterion, eval_func, args, device, return_score=False):
    ...
    if isinstance(model, Mahalanobis):
        ...
        test_ind_score = model.detect(dataset_ind, dataset_ind.splits['test'], device, args).cpu().detach()
    else:
        with torch.no_grad(): 
            test_ind_score = model.detect(dataset_ind, dataset_ind.splits['test'], device, args).cpu().detach()
    if isinstance(dataset_ood, list):
        ...
        for d in dataset_ood:
            if isinstance(model, Mahalanobis):
                test_ood_score = model.detect(dataset_ind, dataset_ind.splits['train'], d, d.node_idx, device, args).cpu().detach()
            elif isinstance(model, ODIN):
                test_ood_score = model.detect(d, d.node_idx, device, args).cpu().detach()
            else:
                with torch.no_grad():
                    test_ood_score = model.detect(d, d.node_idx, device, args).cpu().detach()
        ...
        if isinstance(model, Mahalanobis):
            ...
            test_ood_score = model.detect(dataset_ood, dataset_ood.node_idx, device, args).cpu().detach()
        else:
            with torch.no_grad():
                test_ood_score = model.detect(dataset_ood, dataset_ood.node_idx, device, args).cpu().detach()
        auroc, aupr, fpr, _ = get_measures(test_ind_score, test_ood_score)
